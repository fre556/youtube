...
def enhance_image(image):
    ...
    image = enhancer.enhance(.9)  # Slightly increase brightness
    return image
...
def create_thumbnail_with_effect(image, output_path, config_path, weights_path, classes_path, subscribe_image_path, vintagearchive_image_path, text="Rediscover The Past", font_path="Fonts/Bangers-Regular.ttf"):
    ...
    try:
        ...
        subscribe_image = subscribe_image.resize((300, 200), Image.LANCZOS)
    ...
    # Add the vintage archive image to the bottom right
    try:
        vintagearchive_image = Image.open(vintagearchive_image_path).convert("RGBA")
        vintagearchive_image = vintagearchive_image.resize((300, 300), Image.LANCZOS)
        vintagearchive_position = (image.width - vintagearchive_image.width - 20, image.height - vintagearchive_image.height - 20)
        image.paste(vintagearchive_image, vintagearchive_position, vintagearchive_image)
        print("Vintage archive button added.")
    except Exception as e:
        print(f"Failed to add vintage archive button: {e}")

    image.save(output_path)
    print(f"Thumbnail saved at: {output_path}")

def main(start_label, end_label, video_dir, screenshot_dir, config_path, weights_path, classes_path, subscribe_image_path, vintagearchive_image_path):
    ...
    for label in range(start_label, end_label + 1):
        ...
        if os.path.exists(video_path):
            ...
            if frame is not None:
                ...
                create_thumbnail_with_effect(image, screenshot_path, config_path, weights_path, classes_path, subscribe_image_path, vintagearchive_image_path)
...
if __name__ == "__main__":
    ...
    vintagearchive_image_path = 'Assets/vintagearchive4.png'
    main(start_label, end_label, video_dir, screenshot_dir, config_path, weights_path, classes_path, subscribe_image_path, vintagearchive_image_path)
